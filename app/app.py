...
elif st.session_state.main_page == 'input_user_data':
    st.header("Input User Data")
    uploaded = st.file_uploader("Upload Samsung Health ZIP", type="zip")
    if not uploaded:
        st.info("Please upload a ZIP file.")
        st.stop()

    with zipfile.ZipFile(uploaded) as zf:
        df_food_raw  = load_csv_from_zip(zf, 'food_intake')  or pd.DataFrame()
        df_sleep_raw = load_csv_from_zip(zf, 'sleep')        or pd.DataFrame()
        df_steps_raw = load_csv_from_zip(zf, 'step')         or pd.DataFrame()
        df_water_raw = load_csv_from_zip(zf, 'water_intake') or pd.DataFrame()

    df_food  = clean_food_intake(df_food_raw)   if not df_food_raw.empty  else pd.DataFrame()
    df_sleep = clean_sleep_hours(df_sleep_raw)  if not df_sleep_raw.empty else pd.DataFrame()
    df_steps = clean_step_count(df_steps_raw)   if not df_steps_raw.empty else pd.DataFrame()
    df_water = clean_water_intake(df_water_raw) if not df_water_raw.empty else pd.DataFrame()

    try:
        with zipfile.ZipFile(uploaded) as zf:
            prof = load_csv_from_zip(zf, 'user_profile') or pd.DataFrame()
            default_un = prof.iloc[2,0] if not prof.empty else ""
    except:
        default_un = ""

    new_un = st.text_input("Username:", value=default_un)
    if st.button("Push Data"):
        if not new_un:
            st.error("Username required.")
        else:
            try:
                uid = push_user_data_mysql(
                    new_un, df_food, df_water, df_sleep, df_steps
                )
                st.success(f"MySQL: pushed for user_id={uid}")
                st.session_state.user_id  = uid
            ...
            if st.session_state.user_id:
                try:
                    ingest_user_data_to_neo4j(
                        ...
                        df_food, df_water, df_steps, df_sleep
                    )
                    st.success("Neo4j: ingested")
                except Exception as e:
                    st.error(f"Neo4j error: {e}")

    st.markdown("---")
    with st.expander("Cleaned Preview"):
        if not df_food.empty:  st.write("Food");  st.dataframe(df_food)
        if not df_sleep.empty: st.write("Sleep"); st.dataframe(df_sleep)
        if not df_steps.empty: st.write("Steps"); st.dataframe(df_steps)
        if not df_water.empty: st.write("Water"); st.dataframe(df_water)
